...
const BankHistory = () => {
  ...
  const [items, setItems] = useState<BankIncomeType[]>([]);
  ...
  const fetchIncome = async (bankId: any) => {
    try {
  ...
  const fetchSpend = async (bankId: any) => {
    try {
      const response = await RNFetchBlob.config({trusty: true}).fetch(
        'GET',
  ...
  const initialize = async () => {
    ...
    try {
      const [savedTheme, storedUserID, storedBankID] = await Promise.all([
