...
const EWalletSpend = () => {
  ...
  const selectType = [
    ...
    {key: '11', value: i18n.t('SpendType.Type11')},
    {key: '12', value: i18n.t('SpendType.Type12')},
  ...
  const initialize = async () => {
    ...
    try {
      const [savedTheme, storedUserID, ewalletID] = await Promise.all([
        AsyncStorage.getItem('theme'),
        AsyncStorage.getItem('UserID'),
        AsyncStorage.getItem('EwalletID'),
      ]);
  ...
  const validateNormal = async () => {
    ...
    if (!selectedType) {
      setSelectTypeError(i18n.t('WalletSpend.Type-Empty'));
      isValid = false;
    } else {
      setSelectTypeError('');
    }
    ...
    if (isValid) {
      ...
      if (userID && ewalletID && amount && selectedType && formattedDate) {
        try {
          await RNFetchBlob.config({trusty: true})
            .fetch(
              'POST',
              `${UrlAccess.Url}EwalletSpend/AddSpend`,
              {'Content-Type': 'application/json'},
              JSON.stringify({
                ewalletID: ewalletID,
                userID: userID,
      ...
      if (
        ...
        try {
          await RNFetchBlob.config({trusty: true})
            .fetch(
              'POST',
              `${UrlAccess.Url}EwalletSpend/AddSpendWithBank`,
              {'Content-Type': 'application/json'},
              JSON.stringify({
                ewalletID: ewalletID,
                userID: userID,
      ...
      <KeyboardAvoidingView
        ...
        <View
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              selectedType === '12' ? validateBank() : validateNormal()
            }>
