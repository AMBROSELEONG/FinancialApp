...
const ExpenseDetails = () => {
  ...
  const fetchData = async (debtId: string) => {
    try {
      const response = await RNFetchBlob.config({trusty: true}).fetch(
        'GET',
        `${UrlAccess.Url}Debt/GetDebtById?debtId=${debtId}`,
      ...
      if (result.success === true) {
        ...
        setAmount(result.data.debtAmount);
        setDebtName(result.data.debtName);
        setYear(result.data.year);
        setMonthLeft(result.data.monthLeft);
        ...
        Alert.alert(
          i18n.t('DebtDetail.Congratulation'),
          i18n.t('DebtDetail.Pay-Off'),
          [
            {text: 'OK', onPress: () => navigation.goBack()},
            {text: 'Delete', onPress: () => deleteDebt(debtId)},
          ],
        );
      }
    } catch (error) {
  ...
  const deleteDebt = async (debtId: string) => {
    ...
    try {
      const response = await RNFetchBlob.config({trusty: true}).fetch(
        'DELETE',
        `${UrlAccess.Url}Debt/DeleteDebt?debtId=${debtId}`,
        {'Content-Type': 'application/json'},
      );
  ...
  const MonthLeft = parseInt(monthLeft);
  const remain = parseInt(year) * 12 - MonthLeft;
  const percent = (remain / (parseInt(year) * 12)) * 100;
  const pieData = [
    {value: remain, color: '#3490DE'},
    {value: MonthLeft, color: '#fff'},
  ];

  const debtPay = () => {
    Alert.alert(
      i18n.t('DebtDetail.Pay-Now'),
      i18n.t('DebtDetail.Next-Payment') +
        nextDate +
        '\n' +
  ...
  const paynow = async () => {
    ...
    try {
      await RNFetchBlob.config({trusty: true})
        .fetch(
          'POST',
          `${UrlAccess.Url}Debt/CompletePayment`,
          {'Content-Type': 'application/json'},
          JSON.stringify({
            debtID: debtID,
            userID: userID,
          ...
          <ScrollView>
            ...
            <View
              ...
              <View style={{flex: 2}}>
                <Text style={walletStyle.lowerText}>
                  {i18n.t('DebtDetail.Debt-Pay-Off-Year')}
                </Text>
                <Text
                  style={[
                ...
                <Text style={walletStyle.lowerSubtext2}>
                  {MonthLeft} {i18n.t('DebtDetail.Month-Left')}
                </Text>
              </View>
              <View style={{flex: 1}}>
                <PieChart
            ...
            <View style={{flex: 1, flexDirection: 'row', marginTop: 20}}>
              <View
                ...
                <Text style={walletStyle.lowerText}>
                  {i18n.t('DebtDetail.Cumulative-Repayment')}
                </Text>
              ...
              <View
                ...
                <Text style={walletStyle.lowerText}>
                  {i18n.t('DebtDetail.Estimated-Total')}
                </Text>
            ...
            <View style={{flex: 1, flexDirection: 'row', marginTop: 20}}>
              <View
                ...
                <Text style={walletStyle.lowerText}>
                  {i18n.t('DebtDetail.Start')}
                </Text>
                <Text
                  style={[
                    debtDetailCss.cumulative,
                    {color: isDark ? '#fff' : '#000'},
                  ]}>
                  {Date}
                </Text>
              ...
              <View
                ...
                <Text style={walletStyle.lowerText}>
                  {i18n.t('DebtDetail.Next-Payment-Date')}
                </Text>
                <Text
                  style={[
                    debtDetailCss.cumulative,
